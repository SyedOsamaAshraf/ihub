	...
	if "data" in session:
		...
		if request.method == 'POST':
			...
			if tenth:
				...
				storage.child(f"Certificates/{email}/Education/High School.{file[1]}").put(tenth)
			if twelveth:
				file=twelveth.filename.split('.')
				storage.child(f"Certificates/{email}/Education/Inter.{file[1]}").put(twelveth)
			if ug:
				file=ug.filename.split('.')
				storage.child(f"Certificates/{email}/Education/Under Graduate.{file[1]}").put(ug)
			if pg:
				file=pg.filename.split('.')
				storage.child(f"Certificates/{email}/Education/Post Graduate.{file[1]}").put(pg)
			if phd:
				file=phd.filename.split('.')
				storage.child(f"Certificates/{email}/Education/Phd.{file[1]}").put(phd)
			return redirect(url_for('master'))
		return redirect(url_for('master'))
	return redirect(url_for('login'))
